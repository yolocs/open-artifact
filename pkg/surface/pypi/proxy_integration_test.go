...
// against the real pypi.org (integration tests hit real upstreams). Run it
// locally with:
//
//	go test -tags=integration -run TestPyPIProxyDownload ./pkg/surface/pypi
//
...
// is served from cache. Controllable scenarios (upstream down, stale and
// synthesized fallback, 404/500, filters) are covered by the unit tests in
// proxy_test.go against in-process fakes.
